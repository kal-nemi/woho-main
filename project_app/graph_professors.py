...
class Prof_vis_professors(object):
    ...
    def get_final_graph(self):
        if self.candidate_id:
            self.main_candidate(self.candidate_id, self.experts, self.profs, self.research)
    ...
    def create_graph_viz(self, df_sample, profs, candidate_name, exp_list):
        ...
        for auth, value in df_sample.iterrows():
            print(auth, ' value:', type(value))
            auth_data = profs[profs['created_ID'] == candidate]  # research.iloc[i]['created_ID']
        ...
        '''
        G.nodes[candidate_name]['size'] = 30
        G.nodes[candidate_name]['color'] = 'green'
        G.nodes['Area of Expertise']['size'] = 20
        G.nodes['Area of Expertise']['color'] = 'magenta'
        G.nodes['papers']['size'] = 20
        G.nodes['papers']['color'] = 'blue'
        G.nodes['papers']['title'] = str(df_sample.loc[candidate_name, 'papers'])
        try:
            '''
            area = [s for s in df_sample.columns if 'Area' in s]
            for s in area:
                G.nodes[s]['size'] = 20
                G.nodes[s]['color'] = 'magenta' '''
            for s in exp_list:
                G.nodes[s]['size'] = 20
                G.nodes[s]['color'] = 'red'
    ...
    def visualise_net(self):
        ...
        nt.from_nx(self.G)
        nt.save_graph(r'project_app/templates/pages/graphs/graph_professors_' + str(self.person_id) + '.html')
    ...
    def main_exp(self, expertise, experts, profs, research, df):
        ...
        G = self.create_graph_viz(df_sample, profs, entry_name)
        # Visualise the graph
        self.visualise_net(G)
